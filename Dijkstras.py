...
def dijkstra(adj: [[[]]], n: int): # adj: [[[]]]  ex [[[1, 0], [2, 1]]], returns a list with all number of steps
...
def dijkstra2(adj: [[[]]], n: int): # adj: [[[]]]  ex [[[1, 0], [2, 1]]] where first element in 2-pair is the weight and the second element is the element. Returns a list with all number of steps. Unlike the previous one this also returns a dictionary with previous
    result = [100000] * len(adj)       #arbitrarily big, don't know how to represent infinity
    pred = dict()
    visited = [False] * len(adj)
    #q.remove(n)
